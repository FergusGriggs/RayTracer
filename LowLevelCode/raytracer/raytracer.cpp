...
//[/comment]
Vec3f trace(const Vec3f &rayOrigin, const Vec3f &rayDir, const std::vector<Object*> &objects, int depth)
{
	...
	for (unsigned i = 0; i < objects.size(); ++i)
	{
		...
		if (objects[i]->intersect(rayOrigin, rayDir, t0, t1))
		{
			if (t0 < 0) t0 = t1;
			if (t0 < tnear)
			{
	...
	else
	{
		...
		for (unsigned i = 0; i < objects.size(); ++i)
		{
			if (objects[i]->getMaterial().getEmissiveColour().x > 0)
			{
				// this is a light
				Vec3f transmission = 1;
				Vec3f lightDirection = objects[i]->getPosition() - phit;
				...
				surfaceColor += objectMaterial.getBaseColour() * transmission * std::max(float(0), nhit.dot(lightDirection)) * objects[i]->getMaterial().getEmissiveColour();
			}
		}
	}
...
//[/comment]
void render(const std::vector<Object*> &objects, int frame)
{
	// quick and dirty
	...
	for (unsigned y = 0; y < height; ++y)
	{
		for (unsigned x = 0; x < width; ++x, ++pixel)
		{
			...
			raydir.normalize();
			*pixel = trace(Vec3f(0), raydir, objects, 0);
		}
	}
	// Save result to a PPM image (keep these flags if you compile under Windows)
	std::stringstream ss;
	ss << "./output/spheres" << frame << ".ppm";
...
void BasicRender()
{
	...
	objects.push_back(new Sphere(Vec3f(0.0f, 0.0f, -20.0f), sphereMaterial1, 4.0f));
...
void SmoothScaling()
{
	...
	for (float frame = 0.0f; frame <= 100.0f; frame++)
	{
		objects.push_back(new Sphere(Vec3f(0.0f, -10004.0f, -20.0f), groundMaterial, 10000.0f));
		objects.push_back(new Sphere(Vec3f(0.0f, 0.0f, -20.0f), sphereMaterial1, frame / 100.0f)); // Radius++ change here
		objects.push_back(new Sphere(Vec3f(5.0f, -1.0f, -15.0f), sphereMaterial2, 2.0f));
		objects.push_back(new Sphere(Vec3f(5.0f, 0.0f, -25.0f), sphereMaterial3, 3.0f));
